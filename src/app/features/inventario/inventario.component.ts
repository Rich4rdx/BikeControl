...
export class InventarioComponent implements OnInit {
  ...
  aplicarFiltros() {
    ...
    if (this.filtroTipo !== 'TODAS')
      lista = lista.filter(b => b.tipo === this.filtroTipo);
    ...
    this.bicicletasFiltradas = [
      ...lista.filter(b => b.cantidadInventario > 0),
      ...lista.filter(b => b.cantidadInventario === 0)
    ];
  }

  onSearch(q: string) { this.searchQuery = q; this.aplicarFiltros(); }
  onFiltro(v: string) { this.filtroTipo = v; this.aplicarFiltros(); }

  get totalStock(): number {
    return this.bicicletas.reduce((s, b) => s + b.cantidadInventario, 0);
  }

  get bajosStock(): number {
    return this.bicicletas.filter(b => b.cantidadInventario < 3).length;
  }
  ...
  guardar() {
    ...
    this.bicicletaService.crear(this.form.value).subscribe({
      ...
      error: () => {
        this.toast.error('Error al registrar la bicicleta');
        this.saving = false;
      }
    });
  }
